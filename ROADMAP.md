...
1. **Separate static analysis from query planning.** During `Build`, compile
...
2. **Select the initial operation by estimated total cost.** Compare the cost
   ...
   statistics.
3. **Make execution adaptive after every narrowing step.** Reconsider the
   ...
   allocation-free.
4. **Add representation-specific candidate filters and ordered streaming.**
...
5. **Use a separate order for direct candidate validation.** Once execution
...
6. **Retain bounded per-`Local` plans and results.** Cache query-shape plans,
...
7. **Share sampled planner statistics across `Local` instances.** Only after
...
8. **Control learning bias and memory.** Track sample counts and confidence,
...
9. **Cut over and simplify.** Run the old and new planners against the same
