...
| Operator-specific streaming build при 125% soft target | 10K equality сохранил 9,797 candidates/query и нулевой observed FP rate вместо 10K/1,0 у universal-tail prototype. Production `Index.Search`: 70,030 → 50,386 ns/op; warm Local: 1,386 → 3,893 ns/op, 0 allocs/op. | Принято по жёсткому контракту: полный exact input не удерживается до конца, опубликованный search type не получает wrapper. Рост build cost и Local latency явно принят; `Index.Search` не замедлился. |
