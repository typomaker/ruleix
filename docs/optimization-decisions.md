...
| Однопроходный streaming downgrade при 120% soft target | На 10K/100K equality с четырьмя листьями и 65% budget дал 100% false positives; на 100K занял 2,32 с против 0,43 с exact-first. Ordered-проба при 50–65% не смогла вместить universal tails, хотя exact-first собирался. CPU-профиль 100K отнёс 51,0% cumulative к `representationLadder`, 40,7% к repeated pressure path; `madvise` дал 31,3% flat. | Отклонён как default. Публичный `Build` остаётся exact-first; prototype сохраняется только в benchmark harness для будущего operator-specific или replayable two-pass дизайна. |
