...
| Frozen open-addressed lookup для physical `uint64` equality keys | Budget25 Index rotating: 902,9 → 867,8 ns/op (−3,9%); retained index 1,197 → 0,920 MB (−23%). Budget50 retained 1,913 → 1,642 MB (−14%). Но общий generic `get` перестал встраиваться: exact map microbenchmark 6,11–6,41 → 6,63–6,82 ns/op (+7–8%), inline сначала вырос 2,79 → 3,34 ns/op. | Дополнительная frozen-ветка подняла compiler inline cost до 122 при budget 80. Sentinel, отдельный helper и физически специализированная `uint64` таблица не восстановили оба exact fast path одновременно. При запрете exact-регрессии прототип удалён; `eqRule` не получил mode knowledge, per-Local state или retained cache. |
| Linear-hashing equality levels with a sparse paged offset directory | Apple M1 Max, Go 1.26.0, `GOMAXPROCS=1`, 10K entries, four equality leaves, 50% budget, baseline `9a206c2`, `500ms x5`: HEAD search median 675.5 ns/op, 32 B/op, 2 allocs; linear `map[uint64]` 773.6 ns/op; 256-slot directory 764.9 ns/op; 64-slot directory 760.3 ns/op. Both HEAD and directory returned 1 candidate/query, while accounted usage was 261,234 B and 336,094 B respectively. At 25%, HEAD returned 1.625 candidates/query from 93,670 B; linear hashing returned 2.188 from 160,688 B. Matched `3x x5` Build medians were 38.3 ms/22.0 MB/220K allocs for HEAD and 278.2 ms/170.9 MB/4.42M allocs for the directory candidate. CPU profiles used `-benchtime=3s -cpuprofile`; the new quantizer was only 1.45% flat CPU, so removing per-query mask derivation did not explain or remove the end-to-end regression. | Gradual `2^k+split` bucket counts reduced the observed budget plateau and improved the earlier 55%-budget fixture from 10.19 to 6.81 candidates, but global 1/8 shrink steps required dozens of complete posting rebuilds. Filling the budget also retained more physical map/directory entries even when measured queries were already exact, degrading search locality. The sparse directory recovered only about 1–2% and did not pass the no-search-regression gate. Production changes were removed; exact was not migrated. A future retry must split only collision-bearing buckets without full-generation rebuilds. |
| Four-step intermediate equality bucket ladder | Apple M1 Max, Go 1.26.0, `GOMAXPROCS=1`, baseline `9fd0263`, 10K entries and four equality leaves. The deterministic `100% → 75% → 50% → 37.5% → 25%` ladder improved the 4,827-entry 55%-budget fixture from 21,990 B/10.19 candidates to 27,944 B/9.04 candidates and reduced unused budget from about 31% to 12%. Matched production-shaped `500ms x5` search regressed from median 676.5 to 762.2 ns/op (+12.7%), with unchanged 32 B/op and 2 allocs and one candidate/query. Matched `3x x5` Build regressed from 36.8 ms/22.0 MB/220K allocs to 115.1 ms/79.7 MB/1.78M allocs. CPU profiles used `-benchtime=3s -cpuprofile`; allocation profiles used `-benchtime=1x -memprofile`. Candidate `rebuildEqualityNext` accounted for 62.4 MB cumulative allocation and 68.2% of sampled build allocation space. | A short ladder avoided the earlier 7.3x Build slowdown but still performed multiple whole-generation posting rebuilds and selected a larger physical representation when measured queries were already exact. The candidate therefore improved one boundary fixture without improving production candidate count and violated both search and Build gates. Production changes were removed. Future intermediate precision must be admitted by demonstrated collision reduction and updated without cloning the whole generation. |
| Adaptive equality shard routing lower bound | Apple M1 Max, Go 1.26.0, `GOMAXPROCS=1`, 10K physical `uint64` keys with identical postings, `500ms x7`. General `equalityIndex` sharding regressed lookup from about 7.95 ns to 9.11 ns with 64 shards and 9.23 ns with 256; 1,024 shards reached 8.03 ns because roughly ten entries fit in each small map. A compact `[]map[uint64]uint32` plus shared sets measured 7.77 ns at fixed depth. Adding per-shard variable-depth arithmetic regressed to 8.91 ns; precomputed masks in separate arrays reached 8.38 ns; colocating the map and prepared mask in one shard descriptor reached median 8.07 ns versus 7.95 ns flat (+1.5%), with 0 B/op and 0 allocs throughout. | Adaptive lossy routing has a stable lookup cost even before `All`, local rebuild, policy selection, and metadata accounting. The minimum prepared-mask variant remained slower in all seven paired runs, so it cannot satisfy the no-search-regression requirement. Production integration and exact migration were not attempted. Fixed-depth compact sharding's 2.3% microbenchmark improvement is not evidence for exact adoption: the prototype used hashed `uint64` keys without collision-safe semantic confirmation and should be evaluated separately if exact-only sharding is revisited. |
